...
int main() {
    size_t size;
    char *data = stb_include_load_file("day9.input", &size);
    string_t *lines = slurp(&data);

    point_t *points = malloc(10 * sizeof(point_t));
    for(int i = 0; i < 9; i++){
        points[i].c = '0' + (i + 1);
        points[i].x = SX;
        points[i].y = SY;
    ...
    char *grid = malloc(W * H * sizeof(char));
    ...
    int tx = x;
    int ty = y;
    putCell(grid, 'H', SX, SY);
    drawGrid(grid);
    for(int i = 0; i < arrlen(lines); i++){
        char dir;
        int steps;
        sscanf(lines[i].s, "%c %d", &dir, &steps);
        printf("dir %c %d\n", dir, steps);
        for(int j = 0; j < steps; j++){
            ...
            for(int i = 0; i < 9; i++){
                ...
                //putCell(grid, '.', points[i].x, points[i].y);
                ...
                //putCell(grid, points[i].c, points[i].x, points[i].y);
                if (points[i].c == '9'){
                    putCell(grid, '#', points[i].x, points[i].y);
                }
            }
            //putCell(grid, 'H', x, y);
            //putCell(grid, 's', SX, SY);
            //drawGrid(grid);
        }
    }
    ...
    //         int e = goleft(x, y, lines);
    //         int r = goright(x, y, lines);
    //         int u = goup(x, y, lines);
    //         int d = godown(x, y, lines);
    //         result = max(e * r * u * d, result);
    //     }
    // }
    // printf("result %d\n", result);
}

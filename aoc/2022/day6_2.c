...
void getPacket(char *data, int start, int len, char *packet, int packetSize){
    int j = 0;
    for(int i = start; i < len && i < start + packetSize; i++){
        packet[j++] = data[i];
    }
}

int isSeq(char *packet, int packetSize){
    char buffer[26] = {0};
...
int main() {
    ...
    for(int i = 0; i < size - packetSize; i++){
        ...
        getPacket(data, i, size, packet, packetSize);
        printf("%.*s\n", packetSize, packet);
        if (isSeq(packet, packetSize)){
            printf("got it %d\n", i + packetSize);
            break;
        }
    }

...
#define TOTAL_DISK_SZ (70000000)

int printDirSizes(dir_t *working, int prefix, int needed, int *minDir){
    char s[1024];
    ...
    for(int i = 0; i < arrlen(working->directories); i++){
        ...
        if (sz >= needed){
            if (*minDir == 0)
                *minDir = sz;
            *minDir = min(sz, *minDir);
            printf("%s*%s %d\n", s, working->directories[i].name, sz);
        } else {
            printf("%s%s %d\n", s, working->directories[i].name, sz);
        }
        printDirSizes(&(working->directories[i]), prefix + 2, needed, minDir);
    }
}
...
int main() {
    size_t size;
    char *data = stb_include_load_file("day7.input", &size);
    char store[1024];
    ...
    int total = 0;
    int totalUsedSpace = getDirSize(root);
    int freeSpace = TOTAL_DISK_SZ - totalUsedSpace;
    int spaceNeeded = UPDATE_SIZE - freeSpace;
    printf("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n");
    printf("Free space %d space needed %d\n", freeSpace, spaceNeeded);
    printf("/ %d\n", totalUsedSpace);
    printDirSizes(root, 2, spaceNeeded, &total);
    printf("total: %d\n", total);
